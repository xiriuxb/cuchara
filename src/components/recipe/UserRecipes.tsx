...
export default function UserRecipes({ username }: UserRecipesProps) {
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isPending, error } = useUserRecipes(username);
...
const RecipeCardComponent = ({ url, name, id }: { url: string; name: string; id: string }) => {
  return (
    <Link className="mx-auto relative min-h-auto min-w-36" href={`/recipe/${id}`}>
      <div className="absolute w-full h-full rounded-xl hover:bg-slate-900/25 hover:bg-opacity-35"></div>
      <img className="aspect-square w-full object-cover rounded-xl hover:" src={url} alt={name} />
